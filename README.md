...
OpenCodeMailBridge 是一款基于 PyQt6 的桌面看门狗工具，持续监控指定邮箱的收件箱，匹配特定发件人或命令前缀的邮件，将内容提取为任务指令并发送给本地运行的 OpenCode AI 编码代理（serve 模式）执行，最后将执行结果实时展示在 GUI 日志面板，并可选通过 SMTP 邮件回复给发件人。
